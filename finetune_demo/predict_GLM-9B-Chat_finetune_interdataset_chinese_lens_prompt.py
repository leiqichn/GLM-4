...
model_path = '/root/autodl-tmp/data/ZhipuAI/glm-4-9b-chat'
lora_path = "/root/noval/GLM-4/finetune_demo/output/checkpoint-1000"
...
# 加载模型
model = AutoModelForCausalLM.from_pretrained(model_path, device_map="auto",torch_dtype=torch.bfloat16, trust_remote_code=True).to(device).eval()
    ...
    for i, task in enumerate(tasks):
        ...
        for n in range(num_gen):
            ...
            with torch.no_grad():
                ...
                print(f'### {len(response)}, it:{n}, task:{task} "\n" {response}')
                while (chinese_length(response) < 750 or chinese_length(response) >1500) or is_sentence_repetition(response):
                    ...
                    messages_new = [
                            ...
                            {"role": "user", "content": f"请编写{tasks_prompt[i]}严格要求字数为800字的有完整结尾的小说故事，上一次你的输入少于700或者大于900了。请重新生成。"}]
                    inputs = tokenizer.apply_chat_template(messages_new,
                               add_generation_prompt=True,
                               tokenize=True,
                               return_tensors="pt",
                               return_dict=True
